...
export default function MeseroTab() {
  const { role, orders, getWaiterOrders } = useContext(AppContext);
  ...
  const deliveredOrders = myOrders.filter(o => o.status === 'entregada' && !o.isPaid).sort((a,b) => b.lastUpdated.getTime() - a.lastUpdated.getTime());
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        {deliveredOrders.length > 0 ? (
          ...
          <div className="text-center text-muted-foreground p-8 rounded-lg border-2 border-dashed">
            No hay órdenes entregadas pendientes de pago.
          </div>
        )}
      </div>
