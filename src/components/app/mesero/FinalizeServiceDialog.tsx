...
export default function FinalizeServiceDialog() {
  const { clearWaiterSoldOrders } = useContext(AppContext);
  ...
  const handleConfirm = () => {
    ...
    const correctPassword = passwords['mesero' as UserRole];
    
    if (password === correctPassword) {
      clearWaiterSoldOrders('mesero1');
    ...
    <AlertDialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            Esta acción limpiará tu lista de "Hoy Vendiste" para prepararte para el próximo turno. Las ventas permanecerán en el reporte del jefe. Ingresa tu contraseña para confirmar.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <div className="py-4">
            <Label htmlFor="password">Contraseña de Mesero</Label>
            <Input 
                id="password"
                type="password"
                value={password}
